...
/// A reference to the source of some language item.
///
#[derive(Clone, Copy, Hash, PartialEq, Eq)]
pub enum Source<'a> {
    /// Source is built-in, intrinsic to the language.
    BuiltIn,

    /// Source is a portion of an input file.
    File {
        /// Source file.
        file: &'a File<'a>,
...
impl<'a> BitOr for Source<'a> {
    ...
    /// Returns the smallest `Source<'a>` that includes both `self` and `rhs`.
    ///
    /// # Panics
    /// * `self` and `rhs` reference different files.
    ///
...
impl Pos {
    /// Constructs a new `Pos` at the beginning-of-file position.
    #[inline]
    pub fn bof() -> Self {
        Pos { byte: 0, line: 1, column: 1 }
    }

    /// Advances textual position by one character.
    #[inline]
    ...
    /// Advances textual position by one line.
    #[inline]
